...
public:
    ...
    MLibUSB(const MLibUSB&) = delete;
    MLibUSB(MLibUSB&&) = delete;
    MLibUSB& operator=(const MLibUSB&) = delete;
    MLibUSB& operator=(MLibUSB&&) = delete;

public:
    bool use_usbk;
    libusb_device_handle* _devh;
    quint32 Read(qbyte &output, quint32 size);
    quint32 Write(qbyte input, quint32 size);
...
public:
    ...
    bool filter_bootrom();
    bool LoadLibUSB();
    bool sendUsbTransfer(uint8_t value);
    bool OpenUSBDevice();
    bool BRom_UsbCtrl(uint8_t requesttype, uint8_t request, uint16_t value, uint16_t index, QByteArray &buff, qbool reset = 0);
    bool CloseUSBDevice(qbool reset = 0);
    bool RunProc(QString programm, QStringList args, QByteArray &output);
};
